...
void MobileBilling::removeCustomer(int id)
{
   // std::list<PrepaidCustomer> :: iterator iter;
    ...
    auto iter = std::find_if(cust.begin(),cust.end(),[=]( PrepaidCustomer& l1)
    { 
        return (l1.getCustomerId()==id);
    });
...
PrepaidCustomer* MobileBilling:: findCustomerById(int id)
{
  ...
  auto iter = std::find_if(cust.begin(), cust.end(), [id] ( PrepaidCustomer& s) 
    { 
      return s.getCustomerId() == id; 
    } );
  if(&(*iter)!=NULL)
  {
  return &(*iter);
